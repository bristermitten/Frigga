	...
	fun call(upon: Value?, stack: Stack, context: FriggaContext, params: Map<String, Value>)
	{
		...
		val parameters = signature.params
			.mapValues {
				...
				val providedType = providedParameter.establishType(context)

				val expectedType = it.value.reestablish(context)

				require(expectedType.accepts(providedType)) {
					"Cannot use value of Type $providedType in place of $expectedType for parameter ${it.key}"
				}
		...
		for (it in content)
		{
			...
			{
				if (name != "yield" && name != "break" && name != "if" && name != "Anonymous")
